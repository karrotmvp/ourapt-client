...
export class Class4Api extends runtime.BaseAPI {
   ...
   */
  async getPinnedQuestionOfApartmentUsingGETRaw(
    requestParameters: GetPinnedQuestionOfApartmentUsingGETRequest,
    initOverrides?: RequestInit
    ...
    if (
      ...
      throw new runtime.RequiredError(
        "apartmentId",
        "Required parameter requestParameters.apartmentId was null or undefined when calling getPinnedQuestionOfApartmentUsingGET."
      );
    }
    ...
    const response = await this.request(
      {
        path: `/api/v1/apartment/{apartmentId}/questions/pinned`.replace(
          `{${"apartmentId"}}`,
   ...
   * 사용자에게 보여질 pinned_question 랜덤 조회
   */
  async getPinnedQuestionOfApartmentUsingGET(
    requestParameters: GetPinnedQuestionOfApartmentUsingGETRequest,
    initOverrides?: RequestInit
  ): Promise<CommonResponseBodyOneQuestionDto> {
    const response = await this.getPinnedQuestionOfApartmentUsingGETRaw(
      requestParameters,
      initOverrides
    );
    return await response.value();
